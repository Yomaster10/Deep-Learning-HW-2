...
def mlp_experiment(
    ...
    model = BinaryClassifier(
        model=MLP(
            in_dim=2,
            dims=[*[width, ] * depth, 2],
            nonlins=[*[act, ] * depth, out_act]
        ),
        threshold=threshold,
    )

    grid = dict(lr=lr, weight_decay=weight_decay, momentum=mom, loss_fn=loss_fn)

    loss_fn = grid.pop('loss_fn')
    optimizer = torch.optim.SGD(params=model.parameters(), **grid)
